...
import (
	...
	ttypes "github.com/khulnasoft-lab/vul/pkg/types"
...
func (w *cm) GenerateComplianceReport(ctx context.Context, spec v1alpha1.ReportSpec) error {
	vulResults, err := misconfigReportToVulResults(w.client, ctx)
	if err != nil {
		return err
	}

	status, err := w.buildComplianceReport(spec, vulResults)
	if err != nil {
		return err
	}
...
func (w *cm) buildComplianceReport(spec v1alpha1.ReportSpec, complianceResults []ttypes.Results) (v1alpha1.ReportStatus, error) {
	vulCompSpec := v1alpha1.ToComplainceSpec(spec.Complaince)
	cr, err := report.BuildComplianceReport(complianceResults, vulCompSpec)
	if err != nil {
...
// MisconfigReportToVulResults convert misconfig and infra assessment report Data to vul results
func misconfigReportToVulResults(cli client.Client, ctx context.Context) ([]ttypes.Results, error) {
