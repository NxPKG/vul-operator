...
// metrics on-demand from the vul-operator custom resources. Since these
...
// https://github.com/giantswarm/starboard-exporter, which collects vul
...
func buildMetricDescriptors(config vuloperator.ConfigData) metricDescriptors {
	imageVulnSeverities := map[string]func(vs v1alpha1.VulnerabilitySummary) int{
...
func getDynamicConfigLabels(config vuloperator.ConfigData) []string {
...
func (c ResourcesMetricsCollector) collectVulnerabilityReports(ctx context.Context, metrics chan<- prometheus.Metric, targetNamespaces []string) {
	...
	for _, n := range targetNamespaces {
		...
		for _, r := range reports.Items {
			...
			labelValues[2] = r.Labels[vuloperator.LabelResourceKind]
			labelValues[3] = r.Labels[vuloperator.LabelResourceName]
			labelValues[4] = r.Labels[vuloperator.LabelContainerName]
	...
	for _, n := range targetNamespaces {
		...
		for _, r := range reports.Items {
			if c.Config.MetricsVulnerabilityId {
				...
				vulnLabelValues[2] = r.Labels[vuloperator.LabelResourceKind]
				vulnLabelValues[3] = r.Labels[vuloperator.LabelResourceName]
				vulnLabelValues[4] = r.Labels[vuloperator.LabelContainerName]
...
func (c *ResourcesMetricsCollector) collectClusterRbacAssessmentReports(ctx context.Context, metrics chan<- prometheus.Metric) {
	...
	for _, r := range reports.Items {
		...
		labelValues[1] = r.Labels[vuloperator.LabelResourceKind]
		labelValues[2] = r.Labels[vuloperator.LabelResourceName]
		for i, label := range c.GetReportResourceLabels() {
