...
// Controller for vul-operator-policies-config in the operator namespace; must be cluster scoped even with namespace predicate
	...
	for _, configResource := range resources {
		if err := ctrl.NewControllerManagedBy(mgr).
			For(&corev1.ConfigMap{}, builder.WithPredicates(
				...
				predicate.HasName(vuloperator.PoliciesConfigMapName),
...
func (r *PolicyConfigController) reconcileConfig(kind kube.Kind) reconcile.Func {
	return func(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
		...
		labelSelector, err := labels.Parse(fmt.Sprintf("%s!=%s,%s=%s",
			vuloperator.LabelPluginConfigHash, configHash,
			vuloperator.LabelResourceKind, kind))
		if err != nil {
