...
import (
	...
	"github.com/khulnasoft/vul-operator/pkg/operator/workload"
	...
	for _, resource := range clusterResources {
		...
		if err = ctrl.NewControllerManagedBy(mgr).WithOptions(controller.Options{
			...
			For(resource.ForObject, builder.WithPredicates(
				predicate.Not(predicate.ManagedByVulOperator),
...
func (r *ResourceController) hasClusterReport(ctx context.Context, owner kube.ObjectRef, podSpecHash string, pluginConfigHash string, io rbacassessment.Reader) (bool, error) {
	...
	if report != nil {
		...
		case *v1alpha1.ClusterConfigAuditReport:
			return r.Labels[vuloperator.LabelResourceSpecHash] == podSpecHash &&
				r.Labels[vuloperator.LabelPluginConfigHash] == pluginConfigHash, nil
