...
import (
	...
	control "github.com/khulnasoft/vul-operator/pkg/configauditreport/controller"
...
func (r *TTLReportReconciler) applicableForDeletion(report client.Object, ttlReportAnnotationStr string) bool {
	...
	resourceKind, ok := report.GetLabels()[vuloperator.LabelResourceKind]
	if !ok {
		return false
	}
	policiesHash, ok := report.GetLabels()[vuloperator.LabelPluginConfigHash]
	if !ok {
		return false
	}
