...
// ConfigData holds Vul-operator configuration settings as a set of key-value
// pairs.
...
func GetDefaultConfig() ConfigData {
	return map[string]string{
		keyVulnerabilityReportsScanner:  "Vul",
		keyConfigAuditReportsScanner:    "Vul",
	...
	if err != nil {
		...
		_, err = c.client.CoreV1().ConfigMaps(c.namespace).Create(ctx, &corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				...
				Name:      ConfigMapName,
				Labels: labels.Set{
					LabelK8SAppManagedBy: "vul-operator",
				},
			},
...
func (c *configManager) Delete(ctx context.Context) error {
	...
	err = c.client.CoreV1().ConfigMaps(c.namespace).Delete(ctx, GetPluginConfigMapName("Vul"), metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return err
	}
