...
import (
	...
	"github.com/khulnasoft/vul-operator/pkg/utils"
...
const (
	...
	// SkipDirsAnnotation annotation  example: vul-operator.khulnasoft.github.io/skip-dirs: "/tmp,/home"
	SkipDirsAnnotation = "vul-operator.khulnasoft.github.io/skip-dirs"
	// SkipFilesAnnotation example: vul-operator.khulnasoft.github.io/skip-files: "/src/Gemfile.lock,/examplebinary"
	SkipFilesAnnotation = "vul-operator.khulnasoft.github.io/skip-files"
)
...
func (p *plugin) getCommandAndArgs(ctx vuloperator.PluginContext, mode Mode, imageRef string, vulServerURL string, resultFileName string) ([]string, []string) {
	...
	if mode == ClientServer {
		...
		return []string{"/bin/sh"}, []string{"-c", fmt.Sprintf(`vul image %s '%s' %s %s %s %s %s --cache-dir /tmp/vul/.cache --quiet --format json --server '%s' > /tmp/scan/%s &&  bzip2 -c /tmp/scan/%s | base64`, slow, imageRef, scanners, getSecurityChecks(ctx), imageconfigSecretScannerFlag, vulnTypeFlag, getPkgList(ctx), vulServerURL, resultFileName, resultFileName)}
	}
	...
	return []string{"/bin/sh"}, []string{"-c", fmt.Sprintf(`vul image %s '%s' %s %s %s %s %s --cache-dir /tmp/vul/.cache --quiet %s --format json > /tmp/scan/%s &&  bzip2 -c /tmp/scan/%s | base64`, slow, imageRef, scanners, getSecurityChecks(ctx), imageconfigSecretScannerFlag, vulnTypeFlag, skipUpdate, getPkgList(ctx), resultFileName, resultFileName)}
}
