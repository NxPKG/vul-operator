...
To find vulnerabilities in container images Vul-Operator creates asynchronous
Kubernetes (K8s) Jobs. Even though running a vulnerability scanner as a K8s
Job is expensive, Vul-Operator does not reuse scan results in any way.
For example, if a workload refers to the image that has already been scanned,
Vul-Operator will go ahead and create another (similar) K8s Job.

To some extent, the problem of wasteful and long-running K8s Jobs can be
mitigated by using Vul-Operator with Vul in the [ClientServer] mode instead of
the default [Standalone] mode. In this case a configured Vul server will cache
results of scanning image layers. However, there is still unnecessary overhead
for managing K8s Jobs and communication between Vul client and server.
...
With the proposed cluster-scoped (or global) cache, Vul-Operator can check if the
...
and `bar`. In the current implementation Vul-Operator will spin up two K8s Jobs to
run a scanner and eventually create two VulnerabilityReports in `foo` and `bar`
namespaces respectively.

In a cluster where Vul-Operator is installed for the first time, when we scan the
`nginx` Deployment in the `foo` namespace there's obviously no
ClusterVulnerabilityReport for `nginx:1.16`. Therefore, Vul-Operator will spin up
...
```console
$ vul-operator scan vulnerabilityreports deploy/nginx -n foo -v 3
...
I1008 19:58:19.365190   62385 runnable_job.go:74] Creating job "vul-operator/scan-vulnerabilityreport-cbf8c9b99"
...
I1008 19:58:51.247278   62385 scanner.go:130] Scan job completed: vul-operator/scan-vulnerabilityreport-cbf8c9b99
I1008 19:58:51.247297   62385 scanner.go:262] Getting logs for nginx container in job: vul-operator/scan-vulnerabilityreport-cbf8c9b99
I1008 19:58:51.674449   62385 scanner.go:123] Deleting scan job: vul-operator/scan-vulnerabilityreport-cbf8c9b99
```

Now, if we scan the `nginx` Deployment in the `bar` namespace, Vul-Operator will
...
84bcb5cd46   library/nginx   1.16            Vul     17s   21         50     33       104   0
```

```console
$ vul-operator scan vulnerabilityreports deploy/nginx -n bar -v 3
...
As you can see, Vul-Operator eventually created two VulnerabilityReports by spinning
up only one K8s Job.

```console
$ kubectl get vulnerabilityreports -A
NAMESPACE   NAME                                REPOSITORY      TAG    SCANNER   AGE
bar         replicaset-nginx-6d4cf56db6-nginx   library/nginx   1.16   Vul     5m38s
foo         replicaset-nginx-6d4cf56db6-nginx   library/nginx   1.16   Vul     6m10s
```
...
My suggestion is to solve this problem just like we did in [PR #879](https://github.com/khulnasoft/vul-operator/pull/879).
For each ClusterVulnerabilityReport created we should annotate the report with `vul-operator.khulnasoft.github.io/cluster-vulnerability-report-ttl`.
...
For example: `vul-operator.khulnasoft.github.io/ClusterVulnerabilityReportName: 84bcb5cd46` would make it easy to find.
...
* It's backward compatible and can be implemented as an experimental feature behind
  a gate.
* Both Vul-Operator CLI and Vul-Operator Operator can read and leverage ClusterVulnerabilityReports.

[Standalone]: https://khulnasoft.github.io/vul-operator/v0.15.1/integrations/vulnerability-scanners/vul/#standalone
[ClientServer]: https://khulnasoft.github.io/vul-operator/v0.15.1/integrations/vulnerability-scanners/vul/#clientserver
