...
## if ns.yaml do not exist, cat namespace.yaml to vul-operator.yaml (avoid duplicate namespace definition)
[ ! -f $HELM_TMPDIR/vul-operator/templates/ns.yaml ] && cat $STATIC_DIR/namespace.yaml >> $STATIC_DIR/vul-operator.yaml
...
cp $HELM_TMPDIR/vul-operator/templates/* $STATIC_DIR/
